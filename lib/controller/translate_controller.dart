...
class TranslateController extends GetxController {
  final textC = TextEditingController();
    ...
    final from = ''.obs, to = ''.obs;  //rx variable required to change dynamically
  ...
  Future<void> translate() async {
    if(textC.text.trim().isNotEmpty && to.isNotEmpty)  { //removing white spaces from string from side not centre because user send space not want to send to ai bot 
    status.value= Status.loading;
    String prompt = '';

   if(from.isNotEmpty){
    prompt = 'Can you translate  given text from ${from.value} to ${to.value}:\n ${textC.text}'; //passing text 
    } else {
  ...
  void swapLanguages(){
   ...
   Future <void> googleTranslate() async{
    if(textC.text.trim().isNotEmpty && to.isNotEmpty){
      status.value = Status.loading;
      resultC.text = await APIs.googleTranslate(from: jsonLang[from.value] ?? 'auto', to: jsonLang[to.value] ?? 'en', text: textC.text);
      status.value = Status.complete;
      } else{
        status.value = Status.none;
        if(to.isEmpty) MYDialog.info('Select To Language!');
